...
int mirror2Des = 0;         // Socket descriptor for the mirror
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 5)
    {
        printf("Enter the command in the proper format :%s <IP> <server Port#> <mirror Port#> <mirror Port2#>\n", argv[0]);
        exit(0);
    }
    ...
    // "connecting  to mirror "
    int mirror2_port_number;
    struct sockaddr_in mirror2_address;
    // Creates a socket for the mirror
    if ((mirror2Des = socket(AF_INET, SOCK_STREAM, 0)) < 0)
    {
        fprintf(stderr, "Mirror socket not created.\n");
        exit(1);
    }
    // sets up mirror address information
    mirror2_address.sin_family = AF_INET;
    sscanf(argv[4], "%d", &mirror2_port_number);
    mirror2_address.sin_port = htons((uint16_t)mirror2_port_number);

    if (inet_pton(AF_INET, argv[1], &mirror2_address.sin_addr) < 0)
    {
        fprintf(stderr, "inet_pton() not successful\n");
        exit(2);
    }
    // connects to the mirror.
    if (connect(mirror2Des, (struct sockaddr *)&mirror2_address, sizeof(mirror2_address)) < 0)
    {
        perror("Error Connecting to mirror");
        exit(1);
    }
...
void config_serv_or_mirror()
{
    ...
    if (serv_or_mirror == 1)
    {
        printf("Connected to mirror1\n");
        // set to mirror socket
        process_serv_or_mirror = mirrorDes;
    }
    else if(serv_or_mirror == 2)
    {
        printf("Connected to mirror2\n");
        process_serv_or_mirror = mirror2Des;
    }
    else if(serv_or_mirror == 0)
    {

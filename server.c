...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if ((count>=4 && count<=6) || (count>9 && count%3==2))
        {
            printf("server overload\n");
            printf("Mirror1 will execute\n");
            serv_or_mirror = 1;
        ...
        else if ((count>=7 && count<=9) || (count>9 && count%3==0))
        {
            printf("server overload\n");
            printf("Mirror2 will execute\n");
            serv_or_mirror = 2;
            // forwarding serv_or_mirror flag to client
            send(clientdecsriptor, &serv_or_mirror, sizeof(serv_or_mirror), 0);
            //printf("Informed Client about overload.\n");
        ...
        else if ((count>=1 && count <= 3) || (count>9 && count%3==1))
        {
...
void pclientrequest(int socket){
    ...
    while (1)
    {
        ...
        else if (strncmp(client_rqst, "getfn ", 6) == 0) // check getfn command
        {
            ...
            if (strlen(finalPath) > 0)
            {
                ...
                if (stat(finalPath, &file_stat) == 0)
                {
                    ...
                    char respMsg[512];
                    mode_t filePermissions = file_stat.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO);
                    snprintf(respMsg, sizeof(respMsg), "Filename: %s\nSize: %ld bytes\nDate created: %s\nPermissions: %o",
                             client_rqst + 6, file_stat.st_size, ctime(&file_stat.st_ctime),filePermissions);
        ...
        else if (strncmp(client_rqst, "getfdb ", 7) == 0) //checks if the received command starts with "getfdb ".
        {
